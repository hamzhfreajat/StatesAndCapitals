...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
       ...
       cardinalState = states.stream().filter(i -> i.getStateBird().equals("cardinal")).findFirst().get();
        ...
        isAnyStateLessThan0Elevation = states.stream().anyMatch(index -> index.getLowestElevationInFeet()< 0);
        ...
        isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(index -> index.getHighestElevationInFeet()>21000);
        ...
        doAllStatesHaveAnAnthem = states.stream().allMatch(index -> index.getStateAnthem().isEmpty());
        ...
        doNoStatesHaveAOneWordMotto = states.stream().noneMatch(index -> index.getStateMotto().split(" ").length==1);
        ...
        averageYearlyPrecipitationAcrossStateCapitals = states.stream().map(index -> (double) index.getCapital().getAverageYearlyPrecipitationInInches()).collect(Collectors.averagingDouble(Double::doubleValue));
        ...
        totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(index -> index.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        numberOfStatesByTimeZone = states.stream().collect( Collectors.groupingBy(index -> index.getTimeZones().toString(), Collectors.counting()));
        ...
        stateTreesSortedAscending = states.stream().map(index -> index.getStateTree()).sorted().collect(toList());
        ...
        allStateNamesSemicolonDelimited = states.stream().map(index -> index.getStateName()).collect(joining("; "));
        ...
        allDistinctStateBirds = states.stream().map(index -> index.getStateBird()).distinct().collect(toList());
        System.out.println(allDistinctStateBirds);
        ...
        allDistinctStateBirdsMinusMockingbirds = states.stream().map(index -> index.getStateBird()).filter(index -> !index.contains("mockingbird")).distinct().collect(toList());
        ...
        numberOfDistinctStateBirds = states.stream().map(index -> index.getStateBird()).distinct().collect(counting());
